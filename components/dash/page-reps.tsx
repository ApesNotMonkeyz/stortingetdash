...
export function Reps({ onNav }: { onNav: Nav }) {
  ...
  const filtered = REPRESENTATIVES.filter((r) => {
    ...
    if (sortBy === "voteShare") return b.voteShare - a.voteShare;
    return 0;
  });

  const fylker = Array.from(new Set(REPRESENTATIVES.map((r) => r.fylke))).sort();

  return (
    <Page
      eyebrow="169 mandater"
...
export function RepDetail({ onNav, params }: { onNav: Nav; params: Params }) {
  const rep = repById(params.id) ?? REPRESENTATIVES[0];
  const party = partyById(rep.party);
  const committee = committeeById(rep.committee) ?? { name: "—" };
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 16, color: "var(--fg-muted)", marginTop: 16, maxWidth: 580 }}>
            {rep.role} · {committee.name}. Representant for {rep.fylke}, første periode valgt inn {rep.born + 35}.
          </p>
        </div>
        <div
          style={{
      ...
      >
        {[
          ["Oppmøte", (rep.voteShare * 100).toFixed(0) + "%"],
          ["Stemmer m/parti", "94%"],
          ["Innlegg i sal", "42"],
          ["Forslag fremmet", "8"],
        ].map(([k, v], i) => (
