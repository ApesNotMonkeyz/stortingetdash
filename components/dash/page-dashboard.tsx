...
import type { MergedParty, MergedPartiesEnvelope } from "@/lib/stortinget/types";
...
export function Dashboard({
  ...
  // Only represented parties drive seating visuals — unrepresented ones are
  // still returned by the API for historical context but have seats: 0.
  const seatedParties: Party[] =
    liveParties.status === "ready"
      ? liveParties.data.filter((p) => p.seats > 0)
      : PARTIES;
  const totalSeats = seatedParties.reduce((sum, p) => sum + p.seats, 0);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <section className="card" style={{ padding: 20 }}>
            <div className="eyebrow" style={{ marginBottom: 8 }}>
              Sammensetning · {totalSeats} mandater
            </div>
            <Hemicycle parties={seatedParties} size="md" />
            <div
              style={{
                display: "grid",
