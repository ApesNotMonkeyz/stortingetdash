...
export function Hemicycle({
  ...
  const ordered = sortOrder
    ? (sortOrder.map((id) => parties.find((p) => p.id === id)).filter(Boolean) as Party[])
    : [...parties].sort((a, b) => a.position.econ - b.position.econ);

  const seats: Array<{ partyId: string; color: string; x?: number; y?: number }> = [];
  ordered.forEach((p) => {
    for (let i = 0; i < p.seats; i++) seats.push({ partyId: p.id, color: p.color });
  });
  ...
  const placed: Array<{ partyId: string; color: string; x: number; y: number }> = [];
  let idx = 0;
  for (let r = 0; r < ROWS; r++) {
    const rr = rMin + (rMax - rMin) * (r / (ROWS - 1));
    const n = Math.min(perRow, seats.length - idx);
    const startAngle = Math.PI;
    const endAngle = 0;
    const step = (startAngle - endAngle) / Math.max(1, n - 1);
    for (let i = 0; i < n; i++) {
      const a = startAngle - step * i;
      const x = cx + rr * Math.cos(a);
      const y = cy - rr * Math.sin(a);
      if (idx < seats.length) {
        placed.push({ ...seats[idx], x, y });
        idx++;
      }
    }
  }

  const seatR = compact ? 3 : 4;
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
      {placed.map((s, i) => {
        const dim = highlightId && s.partyId !== highlightId;
        return <circle key={i} cx={s.x} cy={s.y} r={seatR} fill={s.color} opacity={dim ? 0.15 : 1} />;
      })}
    </svg>
  );
}
